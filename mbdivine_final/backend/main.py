...
# ── EMERGENCY PASSWORD RESET (localhost only) ──
@app.post("/api/reset-admin-password")
def reset_admin_password(db=Depends(get_db)):
    new_hash = bcrypt.hashpw("divine123".encode(), bcrypt.gensalt()).decode()
    db.execute("UPDATE users SET password_hash=? WHERE email=?", (new_hash, "admin@mbdivine.com"))
    db.commit()
    return {"success": True, "message": "Password reset to divine123"}
